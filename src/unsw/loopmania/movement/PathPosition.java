...
public class PathPosition {
    ...
    /**
     * Alternative way to create a Path Position
     * @pre: x, y are correct coordinates within the path
     * @post: return an ordered path with the correct index within the path for the given coordinates
     * @param x the x coord in the path
     * @param y the y coord in the path
    ...
    public PathPosition(int x, int y, List<Pair<Integer, Integer>> orderedPath){
        ...
        // update internal property
        resetPosiitionInPathBasedOnCoordinates();
    }
     ...
     * the current position in the path, and the ordered path.
     */
    private void resetPosiitionInPathBasedOnCoordinates(){
        this.currentPositionInPath = orderedPath.indexOf(new Pair<Integer, Integer>(x.get(), y.get()));
    }
